...
{
	//echo "{$i}:".count( $comment[$i] )."<br/>\n";
	$index[$i] = rand( 0, count($comment[$i])-1 );
}
*/

foreach( $comment as $i => $v ){
	//echo "{$i}:".count($comment[$i])."<br/>\n";
	$index[$i] = rand( 0, count($comment[$i])-1 );
}

/*
$when_index =  rand(0, count($comment[0]) - 1 );
$who_index =  rand(0, count($who) - 1 );
$how_index =  rand(0, count($how) - 1);
*/
...
<script async src="https://www.googletagmanager.com/gtag/js?id=G-8GDG3PH10P"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-8GDG3PH10P');
</script>
</body>
</html>